 ...
 */
        ...
        double & coeff_t)
    {
        ...
        pair<SMatrixXd, VectorXd> CE_0 = 
            getConstraintsEndpoints_D(T, B_ori, E, 
                    p_s, vel(0), acc(0),
                    p_t, vel(1), acc(1));
    ...
    VectorXd getMovedPolynomial(int N, const VectorXd & p, double d_t)
    {
        ...
        for (int i = 0; i < n; ++i)
        {
            ...
            for (int j = 0; j <= i; ++j)
            {
                ret(i - j) += nchoosek(i, j) * p(i) * t_acc;
                t_acc *= d_t; 
            }
        }
        return ret;
    ...
    VectorXd getMovedPolynomial(
            const TrajectoryGenerator::DimensionConfig &config)
    {
        double t = config.traj_time;
        VectorXd ret = VectorXd::Zero(config.N * config.M);
        for (int i = 0; i < config.M; ++i)
        {
            ret.segment(i * config.N, config.N) =
                getMovedPolynomial(config.N, config.traj, t);
            t += config.time(i);
        } 
        return ret;
    }

    inline SMatrixXd getTrackingErrorMatrix(int N, double t)
    {
        SMatrixXd ret(N, N);
        for (int i = 0; i < N; ++i)
        {
            for (int j = 0; j < N; ++j)
                ret.insert(i, j) = pow(t, i + j + 1) / (i + j + 1);
        }

        return ret;
    }

    SMatrixXd getTrackingErrorMatrix(
            const TrajectoryGenerator::DimensionConfig &config)
    {
        SMatrixXd ret(config.M * config.N, config.M * config.N);

        for (int i = 0; i < config.M; ++i)
        {
            insertBlock(ret, i * config.N, i * config.N, 
                    getTrackingErrorMatrix(config.N, config.time(i)));
        }
        return ret;
    }

    VectorXd getTimeAllocation(
            const TrajectoryGenerator::TrackingCorridorConfig &config)
    {
        auto & face = config.face;
        auto & t_beg = config.t_beg, & t_end = config.t_end;

        VectorXd ret = VectorXd::Zero(config.M);
        if (config.M == 1) 
        {
            ret << (config.t_end - config.t_beg);
            return ret;
        }
        
        auto getFaceCenter = [&face](int id) -> VectorXd
        {
            VectorXd ret(_TOT_DIM);
            ret <<
                (face(id, _BDY_x) + face(id, _BDY_X)) * 0.5,
                (face(id, _BDY_y) + face(id, _BDY_Y)) * 0.5,
                (face(id, _BDY_z) + face(id, _BDY_Z)) * 0.5;
            return ret;
        };
    ...
    VectorXd getTimeAllocation(
            const vector<pair<double, double> > & stamps, 
            const MatrixXd &face)
    {
        assert(face.rows() + 1 == stamps.size());

        VectorXd ret = VectorXd::Zero(stamps.size());

        auto isLegal = [&stamps](int id) 
        {
            return stamps[id].first <= stamps[id].second;
        };
        ...
        auto allocateTime = [&](int L, int R, double t)->void
        {
            VectorXd len(R - L + 1);
            for (int i = L ; i <= R; ++i)
                len[i - L] = (getFaceCenter(i - 1) - getFaceCenter(i)).norm();
            double len_sum = len.sum();
            for (int i = 0; i < len.rows(); ++i)
                ret[L + i] = t * len[i] / len_sum;
        };

        for (int idx = 0; idx < (int)stamps.size(); ++idx)
        {
            if (isLegal(idx))
                ret[idx] = stamps[idx].second - stamps[idx].first;
            else
            {
                int L = idx, R = idx;
                while (R + 1 < (int)stamps.size() && !isLegal(R + 1)) R += 1;

                assert(L > 0 && R + 1< stamps.size());

                allocateTime(L, R, stamps[R + 1].first - stamps[L - 1].second);
                idx = R;
            }
        }
    ...
    bool TrajectoryGenerator::TrackingCorridorConfig::isLegal()
    {
        assert(traj.cols() == _TOT_DIM);
        assert(grid.cols() == _TOT_BDY);
        assert(face.cols() == _TOT_BDY);
        assert(uninflated_grid.cols() == _TOT_BDY);

        assert(grid.rows() == M);
        assert(face.rows() + 1 == M);
        assert(uninflated_grid.rows() == M);

        assert(init_state.rows() == 3);
        assert(dest_state.rows() == 3);
        assert(init_state.cols() == _TOT_DIM);
        assert(dest_state.cols() == _TOT_DIM);
    ...
    TrajectoryGenerator::DimensionConfig 
        TrajectoryGenerator::TrackingCorridorConfig::getDimensionConfig(int dim)
    {
        ...
        ret.time = this->time;
        ret.grid = this->grid.block(0, dim << 1, this->M, 2);
        ret.face = this->face.block(0, dim << 1, this->M - 1, 2);
        //ret.uninflated_grid = this->uninflated_grid.block(0, dim << 1, this->M, 2);

        ret.M = this->M;
        ret.N = this->N;
        ret.R = this->R;

        ret.max_acc = this->max_acc;
        ret.max_vel = this->max_vel;
        return ret;
    }

    pair<SMatrixXd, VectorXd> getConstraintsEndpoints_D(
        const TrajectoryGenerator::DimensionConfig & config)
    {
        SMatrixXd CE(R + R - 1, (config.M + 1) * config.R);
        CE.reserve(R + R - 1);
        VectorXd b = VectorXd::Zero(R + R - 1);

        b << config.init_state, 0.0, 0.0;

        for (int j = 0; j < R; ++j)
        {
            CE.insert(j, 0 * config.R + j) = 1.0;
        }
        
        for (int j = 1; j < R; ++j)
        {
            CE.insert(R + j - 1, config.M * config.R + j) = 1.0;
        }

        return make_pair(CE, b);
    }

    pair<SMatrixXd, VectorXd> getConstraintsDestination(
            const TrajectoryGenerator::DimensionConfig &config)
    {
        SMatrixXd CI(2, (config.M + 1) * config.R);
        CI.insert(0, config.M * config.R + 0) = -1.0;
        CI.insert(1, config.M * config.R + 0) = 1.0;
        VectorXd b(2);
        b << 
            -config.grid(config.M - 1, 0),
            config.grid(config.M - 1, 1);
        return make_pair(CI, b);
    }

    VectorXd getTrackingTrajectoryByDim(
            const TrajectoryGenerator::DimensionConfig & config)
    {
        auto & E = config.face;
        auto & B = config.grid;
        ...
        //> Hessian Matrix 
        SMatrixXd Q_DD, Q_FD, Q = getCostMatrix(T);
        SMatrixXd H = getTrackingErrorMatrix(config);
        MatrixXd COST;
        VectorXd c = getMovedPolynomial(config);
        //return c;
        ...
        //> Endpoints
        pair<SMatrixXd, VectorXd> CE_0 =
            getConstraintsEndpoints_D(config);

        pair<SMatrixXd, VectorXd> CI_0 = 
            getConstraintsDestination(config);

        // clog << "2. Endpoints." << endl;
        // Corridors
        pair<pair<SMatrixXd, VectorXd>, pair<SMatrixXd, VectorXd> > CE_CI_1 = 
            getConstrainsOverlappedCorridors_D(T , B, E);
        CE_CI_1.second = combineRowsPr(CE_CI_1.second, CI_0);
        ...
        { // deal with the cost matrix
            auto A = IP2D * D2D.transpose() * RD.transpose();
            SMatrixXd QQ = A.transpose() * Q * A;
            Q_DD = QQ.block(0, 0, n_d, n_d);
            Q_FD = QQ.block(n_d, 0, n_f, n_d);
            c = -1.0 * RDD * D2D * IP2D.transpose() * H * c;
        }
    ...
    TrajectoryGenerator::TrajectoryConfig
        TrajectoryGenerator::getTrackingTrajectory(
            TrajectoryGenerator::TrackingCorridorConfig & config)
    {
        assert(config.isLegal());
        ...
        f_vel = config.f_vel;
        f_acc = config.f_acc;
        //ret.time = config.time = getTime_smart(
        //        config.init_state.row(0).transpose(),
        //        config.dest_state.row(0).transpose(),
        //        config.face,
        //        MatrixXd::Zero(3, 3));
        ret.coef.resize(config.M * config.N, _TOT_DIM);

        // ROS_WARN("[TRAJ_GEN] 1. got the time allocation ");
        for (int dim = _DIM_x; dim < _TOT_DIM; ++dim)
        {
            _error_code = 0;
            ret.coef.block(0, dim, config.M * config.N, 1) = 
                getTrackingTrajectoryByDim(config.getDimensionConfig(dim));
            if (_error_code) ret.error_code = _error_code;
        }

        // ROS_WARN("[TRAJ_GEN] 2. got the tracking trajectory.");
        return ret;
    }
}
