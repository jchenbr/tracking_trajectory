...
{
    static multimap<double, EdgePtr> weight_pedge;
    static vector<EdgePtr> id_edge(_RESERVE_SIZE);
    static vector<double> opt_val(_RESERVE_SIZE);
    static vector<double> heu_val(_RESERVE_SIZE);
    static vector<NodePtr> lst_node(_RESERVE_SIZE);
    static vector<EdgePtr> lst_edge(_RESERVE_SIZE);
...
pair<Eigen::MatrixXd, Eigen::MatrixXd> VoxelGraph::getPathAStar(
        const vector<EdgePtr> & start_edges, EdgePtr p_target_edge)
{
    assert(!start_edges.empty());
    static multimap<double, EdgePtr> weight_pedge;
    static vector<EdgePtr> id_edge(_RESERVE_SIZE);
    static vector<double> opt_val(_RESERVE_SIZE);
    static vector<double> heu_val(_RESERVE_SIZE);
    ...
    while (!weight_pedge.empty())
    {
        ...
        if (p_target_edge->id != _EDGE_ID_UNKNOWN) break;
    }

    if (p_target_edge->id == _EDGE_ID_UNKNOWN) 
        return make_pair(Eigen::MatrixXd(0, 0), Eigen::MatrixXd(0, 0));

    // calculate the matrix output 
    int M = 0;

    for (u = p_target_edge; lst_edge[u->id] != NULL; u = lst_edge[u->id]) M += 1;

    Eigen::MatrixXd node(_TOT_BDY, M), edge(_TOT_BDY, M + 1);

    int m = 1;
    for (u = p_target_edge; lst_edge[u->id] != NULL; u = lst_edge[u->id])
    {
        edge.col(M + 1 - m) << 
            u->bdy[_BDY_x], u->bdy[_BDY_X],
            u->bdy[_BDY_y], u->bdy[_BDY_Y],
